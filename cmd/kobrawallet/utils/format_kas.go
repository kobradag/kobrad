...
// FormatKobra takes the amount of leors as uint64, and returns amount of KODA with 8  decimal places
func FormatKobra(amount uint64) string {
	res := "                   "
	if amount > 0 {
		res = fmt.Sprintf("%19.8f", float64(amount)/constants.LeorPerKobra)
	}
	return res
}
// KobraToLeor takes in a string representation of the Kas value to convert to Sompi
func KobraToLeor(amount string) (uint64, error) {
	...
	amountStr := ""
	if constants.LeorPerKobra%10 != 0 {
		return 0, errors.Errorf("Unable to convert to leor when LeorPerKobra is not a multiple of 10")
	}
	decimalPlaces := int(math.Log10(constants.LeorPerKobra))
	decimalStr := ""
	if len(parts) == 2 {
		decimalStr = parts[1]
	}
